...
// Отправить уведомление о завершении турнира в группу
async function sendTournamentWinnerNotification(tournamentName, winnerUsername) {
  const TELEGRAM_BOT_TOKEN = process.env.TELEGRAM_BOT_TOKEN;
  ...
  try {
    ...
    const user = db
      .prepare("SELECT id, username, telegram_username FROM users WHERE username = ?")
      .get(winnerUsername);
    ...
    if (user && user.telegram_username) {
      ...
      console.log(`🔍 Поиск telegram пользователя для @${searchedUsername}, результат:`, telegramUser);
    } else {
      ...
      console.log(`🔍 Прямой поиск telegram пользователя: ${cleanUsername}, результат:`, telegramUser);
    }
    ...
    if (telegramUser && telegramUser.chat_id) {
      console.log(`✅ Найден telegram пользователь: @${searchedUsername} (chat_id: ${telegramUser.chat_id})`);
    ...
    messageText += `\n\n\n\n\n👥 <b>Участвовал</b>\n🏁 <b>Последнее место:</b> ${lastPlace ? lastPlace.username : "—"}`;
    if (lastPlace) {
    ...
    for (const chatId of chatIds) {
      try {
        ...
        console.log(`✅ Уведомление о завершении турнира отправлено в группу ${chatId}`);
      } catch (err) {
        console.error(`❌ Ошибка при отправке уведомления о турнире в группу ${chatId}:`, err.message);
      }
    }
  } catch (error) {
    console.error("❌ Ошибка отправки уведомления о завершении турнира:", error);
  }
}
...
export {
  ...
  sendTournamentWinnerNotification,
