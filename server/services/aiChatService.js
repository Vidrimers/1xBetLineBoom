...
`;
  ...
  if (dbContext.periodContext) {
    prompt = `🚨 КРИТИЧЕСКИ ВАЖНО - ПРИОРИТЕТНАЯ ИНФОРМАЦИЯ:\n${dbContext.periodContext}\n\n` +
             `ИНСТРУКЦИЯ: Пользователь спрашивает про КОНКРЕТНЫЙ ПЕРИОД и КОНКРЕТНЫЙ ТУРНИР.\n` +
             `Отвечай ТОЛЬКО на основе информации выше про этот период.\n` +
             `Если у пользователя НЕТ ставок в этом периоде - так и скажи прямо: "не делал ставки на матчи этой даты".\n` +
             `НЕ говори про общую статистику по турниру, НЕ говори про другие периоды.\n\n` +
             prompt;
  }
