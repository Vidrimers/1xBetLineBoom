...
export async function openConfigureCategoriesModal() {
  ...
  modal.innerHTML = `
    <div style="background:#1e2a3a;padding:30px;border-radius:12px;max-width:900px;width:95%;max-height:90vh;overflow-y:auto;box-shadow:0 4px 20px rgba(0,0,0,0.3);">
      <h3 style="margin:0 0 20px 0;color:#5a9fd4;"><svg class="icon" aria-hidden="true"><use href="#icon-settings"></use></svg> Настройка категорий админ-панели</h3>
      ...
      <div style="background:rgba(30,35,45,0.5);border:1px solid rgba(90,159,212,0.3);border-radius:8px;padding:15px;">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;">
          ...
          <div style="display:flex;gap:5px;margin-left:10px;">
            ${index > 0 ? '<button onclick="moveCategoryUp(' + index + ')" style="padding:8px 12px;background:rgba(90,159,212,0.7);color:#e0e6f0;border:1px solid #3a7bd5;border-radius:4px;cursor:pointer;font-size:0.9em;" title="Переместить вверх">⬆</button>' : ''}
            ${index < currentEditingConfig.categories.length - 1 ? '<button onclick="moveCategoryDown(' + index + ')" style="padding:8px 12px;background:rgba(90,159,212,0.7);color:#e0e6f0;border:1px solid #3a7bd5;border-radius:4px;cursor:pointer;font-size:0.9em;" title="Переместить вниз">⬇</button>' : ''}
            <button onclick="deleteCategory(${index})" style="padding:8px 12px;background:rgba(244,67,54,0.7);color:#ffb3b3;border:1px solid #f44336;border-radius:4px;cursor:pointer;font-size:0.9em;" title="Удалить категорию"><svg class="icon" aria-label="Удалить"><use href="#icon-delete"></use></svg></button>
          </div>
        </div>
...
export function renderButtonsTab() {
  ...
  let html = `
    <div style="background:rgba(255,152,0,0.2);border-left:4px solid #ff9800;padding:15px;border-radius:4px;margin-bottom:15px;color:#ffe0b2;font-size:0.9em;">
      <svg class="icon" aria-hidden="true"><use href="#icon-settings"></use></svg> Выберите категорию для каждой кнопки. Кнопки будут отображаться в выбранной категории.
    </div>
    <div style="display:flex;flex-direction:column;gap:10px;">
  `;
  ...
  return `
    <div style="text-align:center;padding:40px;">
      <div style="background:rgba(244,67,54,0.2);border:2px solid #f44336;border-radius:12px;padding:30px;margin-bottom:20px;">
        <div style="font-size:3em;margin-bottom:15px;"><svg class="icon" aria-hidden="true"><use href="#icon-warning"></use></svg></div>
      ...
      <div style="color:#b0b8c8;font-size:0.9em;line-height:1.6;"><p style="margin:0;">Дефолтная конфигурация включает 6 категорий:<br/><svg class="icon" aria-hidden="true"><use href="#icon-stats"></use></svg> Система и логи, <svg class="icon" aria-hidden="true"><use href="#icon-participants"></use></svg> Пользователи и модерация,<br/><svg class="icon" aria-hidden="true"><use href="#icon-manual"></use></svg> Контент и новости, <svg class="icon" aria-hidden="true"><use href="#icon-settings"></use></svg> Настройки интерфейса,<br/><svg class="icon" aria-hidden="true"><use href="#icon-bell"></use></svg> Уведомления, <svg class="icon" aria-hidden="true"><use href="#icon-tools"></use></svg> Утилиты и инструменты</p></div>
    </div>
  `;
}
...
export async function deleteCategory(index) {
  ...
  if (buttonCount > 0) {
    const confirmed = await showCustomConfirm(
      ...
      'Удаление категории', '<svg class="icon" aria-hidden="true"><use href="#icon-warning"></use></svg>'
    );
...
export async function resetToDefaultConfig() {
  const confirmed = await showCustomConfirm(
    ...
    'Подтверждение сброса', '<svg class="icon" aria-hidden="true"><use href="#icon-warning"></use></svg>'
  );
