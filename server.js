...
// terminalLogs, addTerminalLog — перенесены в server/utils/logger.js
...
// Загружаем логи из файла при запуске — перенесено в server/utils/logger.js (initTerminalLogs)
...
// writeBetLog — перенесена в server/utils/logger.js
...
// resetLogFile — перенесена в server/utils/logger.js
...
app.post("/api/ai-chat", async (req, res) => {
  try {
    const { messages, action, actionData, username, context } = req.body;
    ...
    if (!username) {
      return res.json({
        text: 'Извини, я не отвечаю незнакомым персонажам 🤷‍♂️ Войди в систему, чтобы я мог тебе помочь!',
        provider: 'system'
      });
    }
...
${match.predictors ? `Прогнозы сделали: ${match.predictors}` : ''}
    ...
    // Если передан контекст страницы, используем его для фильтрации
    let currentEventId = null;
    let contextInfo = '';
    
    if (context && context.event) {
      currentEventId = context.event.id;
      contextInfo = `Пользователь сейчас смотрит турнир: ${context.event.name}`;
      
      if (context.round) {
        contextInfo += `\nТекущий тур: ${context.round}`;
      }
      
      if (context.modal === 'bracket' && context.bracket) {
        contextInfo += `\nОткрыта сетка: ${context.bracket.name}`;
      } else if (context.modal === 'tournament') {
        contextInfo += `\nОткрыто модальное окно турнира`;
      } else if (context.modal === 'settings') {
        contextInfo += `\nПользователь в настройках`;
      }
    }
    
    // Получаем участников турнира (используем контекст если есть)
    let allParticipants = [];
    let userStats = null;
    let remainingMatches = 0;
    
    const targetEventId = currentEventId || (events.length > 0 ? events[0].id : null);
    
    if (targetEventId) {
      allParticipants = getTournamentParticipants(db, targetEventId);
      remainingMatches = getRemainingMatches(db, targetEventId);
      
      if (username) {
        userStats = getUserTournamentStats(db, targetEventId, username);
      }
      
      console.log(`📊 Используется турнир ID ${targetEventId}:`, events.find(e => e.id === targetEventId)?.name);
    } else if (!targetEventId && events.length === 0) {
      // Если нет активных турниров вообще
      console.log('⚠️ Нет активных турниров');
    } else if (!targetEventId && events.length > 0) {
      // Если турнир не выбран, но есть активные - берём первый
      console.log('⚠️ Турнир не выбран, используем первый активный:', events[0].name);
    }
    ...
    if (targetEventId) {
      const brackets = getTournamentBrackets(db, targetEventId);
    ...
    if ((lowerMsg.includes('ставк') || lowerMsg.includes('прогноз') || lowerMsg.includes('выбор')) && targetEventId) {
      const words = lastMessage.content.split(/\s+/);
      for (const word of words) {
        if (word.length > 2 && (!username || word.toLowerCase() !== username.toLowerCase())) {
          const betsResult = getUserBets(db, targetEventId, word, username);
    ...
    if ((lowerMsg.includes('догна') || lowerMsg.includes('обогна') || lowerMsg.includes('сравн')) && username && targetEventId) {
      // Пытаемся найти имя другого пользователя в сообщении
      const words = lastMessage.content.split(/\s+/);
      for (const word of words) {
        if (word.length > 2 && word !== username) {
          const comparison = compareUsers(db, targetEventId, username, word);
          if (comparison) {
            comparisonContext = `
...
${comparison.canCatchUp ? '✅ Догнать возможно!' : '⚠️ Догнать будет сложно'}
    ...
    const eventsContext = events.length > 0
      ? events.map(e => `${e.name} (${e.description || 'Турнир'}) - ${e.status === 'active' ? 'Активен' : 'Завершён'}`).join('\n')
      : '';
...
Ожидает результатов: ${userStats.pending_bets}`
    ...
    // Формируем контекст для AI
    const selectedTournament = targetEventId ? events.find(e => e.id === targetEventId) : null;
    const tournamentInfo = selectedTournament 
      ? `\n🏆 ТЕКУЩИЙ ТУРНИР: ${selectedTournament.name}\n` 
      : '';
    ...
    const aiContext = { 
      ...
      pageContext: contextInfo + tournamentInfo,
      remainingMatches: remainingMatches > 0 ? `Осталось матчей в турнире: ${remainingMatches}` : ''
    };

    // Логируем что передаём в AI (для отладки)
    console.log('🤖 AI Chat - Контекст для AI:');
    console.log('  - События:', eventsContext ? 'Да ✓' : 'Нет ✗');
    console.log('  - Участники:', participantsContext ? `Да ✓ (${allParticipants.length} чел.)` : 'Нет ✗');
    console.log('  - Статистика юзера:', userStatsContext ? 'Да ✓' : 'Нет ✗');
    console.log('  - Сравнение:', comparisonContext ? 'Да ✓' : 'Нет ✗');
    console.log('  - Контекст страницы:', contextInfo ? 'Да ✓' : 'Нет ✗');
    if (participantsContext) {
      console.log('  - Первые 3 участника:', allParticipants.slice(0, 3).map(p => `${p.username} (${p.total_points} очков)`).join(', '));
    }

    // Отправляем в AI
    const aiResponse = await sendToAI(messages, aiContext);

    // Проверяем ответ AI - если он говорит что нет доступа, но данные есть - исправляем
    const responseText = aiResponse.text ? aiResponse.text.toLowerCase() : '';
    const hasWrongResponse = responseText.includes('не имею доступ') || 
                            responseText.includes('не могу посмотреть') ||
                            responseText.includes('нет данных') ||
                            responseText.includes('не вижу таблиц') ||
                            responseText.includes('нет таблиц');
    
    if (aiResponse.text && participantsContext && hasWrongResponse) {
      
      console.log('⚠️ AI ответил неправильно, хотя данные есть. Исправляю ответ...');
      console.log('   Участников в базе:', allParticipants.length);
      console.log('   Статистика юзера:', userStats ? 'Есть' : 'Нет');
      
      // Формируем правильный ответ на основе данных
      if (lowerMsg.includes('лидир') || lowerMsg.includes('первое место') || lowerMsg.includes('кто впереди')) {
        const leader = allParticipants[0];
        aiResponse.text = `Лидирует ${leader.username} с ${leader.total_points} очками! 🏆 У него ${leader.exact_predictions} точных прогнозов.`;
      } else if (username && (lowerMsg.includes('мои очки') || lowerMsg.includes('моя позиция') || lowerMsg.includes('мое место') || lowerMsg.includes('сколько у меня'))) {
        if (userStats) {
          aiResponse.text = `У тебя ${userStats.total_points} очков, ты на ${userStats.position} месте 📊 Сделано ${userStats.total_bets} прогнозов (${userStats.exact_predictions} точных).`;
        } else {
          // Ищем пользователя в общем списке участников
          const userInList = allParticipants.find(p => p.username.toLowerCase() === username.toLowerCase());
          if (userInList) {
            const position = allParticipants.indexOf(userInList) + 1;
            aiResponse.text = `У тебя ${userInList.total_points} очков, ты на ${position} месте 📊 Сделано ${userInList.total_bets} прогнозов (${userInList.exact_predictions} точных).`;
          } else {
            aiResponse.text = `Похоже, ты ещё не делал прогнозов в этом турнире 🤔`;
          }
        }
      } else {
        // Общий ответ с топ-3
        const top3 = allParticipants.slice(0, 3).map((p, i) => 
          `${i + 1}. ${p.username}: ${p.total_points} очков`
        ).join('\n');
        aiResponse.text = `Вот топ-3 участников турнира:\n${top3}\n\nВсего участников: ${allParticipants.length} 📊`;
      }
    }
